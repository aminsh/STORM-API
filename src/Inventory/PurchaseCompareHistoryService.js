...
@injectable()
export class PurchaseCompareHistoryService {
    ...
    createOutput(invoice) {
        ...
        ids.forEach(id => this.inventoryService.setInvoice(id, invoice, 'outputBackFromPurchase'));
    }
    ...
    createInput(lines, invoice) {
        ...
        ids.forEach(id => this.inventoryService.setInvoice(id, invoice, 'inputPurchase'));
    }
        ...
        let invoiceLines = invoice.invoiceLines.asEnumerable().where(e => this.productRepository.isGood(e.productId)).toArray(),
            inventoryLines = invoice.inventories.asEnumerable()
                ...
                .groupBy(
                    ...
                    (productId, items) => ({
                        productId,
                        quantity: items.sum(e => e.type === 'input' ? e.quantity : e.quantity * -1),
