...
export class PaypingInterfacePaymentGateway {
    ...
    _setServiceToken() {
        ...
        try {
            ...
            toResult(this.persistedConfigService.set(this.serviceTokenKey, value));
        }
        catch (e) {
            throw new Error(e);
        }
     ...
     */
    get _serviceToken() {
        let serviceToken = toResult(this.persistedConfigService.get(this.serviceTokenKey));

        if (!serviceToken) {
            this._setServiceToken();
            serviceToken = toResult(this.persistedConfigService.get(this.serviceTokenKey));
        }
